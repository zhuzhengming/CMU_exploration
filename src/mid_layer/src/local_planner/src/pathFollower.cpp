...
int main(int argc, char** argv)
{
  ...
  // For GO2
  ros::Publisher pubSpeed_only = nh.advertise<geometry_msgs::Twist> ("/cmd_vel_only", 5);
  geometry_msgs::Twist cmd_vel_only;
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      if (pubSkipCount < 0) {
        ...
        // For GO2
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) cmd_vel_only.linear.x = 0;
        else cmd_vel_only.linear.x = vehicleSpeed;
        cmd_vel_only.angular.z = vehicleYawRate;
        pubSpeed_only.publish(cmd_vel_only);
