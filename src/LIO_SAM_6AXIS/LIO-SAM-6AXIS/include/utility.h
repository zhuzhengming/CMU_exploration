...
//        nh.param<std::string>("lio_sam_6axis/sensor", sensorStr, "ouster");
        nh.param<std::string>("lio_sam_6axis/sensor", sensorStr, "hesai");
        ...
        nh.param<int>("lio_sam_6axis/imu_type", imuType, 0);
        nh.param<int>("lio_sam_6axis/imu_sensor", imuSensor, 1);
    ...
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu &imu_in) {
        ...
        // std::cout<<"acc1 is "<< acc.transpose() <<std::endl;


        //livox 内置的六轴imu的加速度单位是g 这里要还原到m/s^2
        if((imuType==0) && (imuSensor==0))
        {
            acc.normalize();
            acc*=imuGravity;
            // std::cout<<"acc2 is "<< acc.transpose() <<std::endl;
        }
...
sensor_msgs::PointCloud2 publishCloud(const ros::Publisher &thisPub,
    ...
    std::cout<< "here is sensor_msgs::PointCloud2 publishCloud function _________"<<thisPub.getTopic()<<std::endl;
    return tempCloud;
}
